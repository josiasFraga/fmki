...
class LocalizationHelper extends Helper
{
    ...
    private function injectScripts()
    {
        ...
        $scripts = array_merge($scripts, ['Localization.cidades']);
    ...
    private function searchCities()
    {
        ...
        // Set state caught id
        $this->stateId = $city->estado_id;

    	// Search for all states
    	$cities = $citiesTable->find('list', [
    		'keyField' => 'id',
    		'valueField' => 'name'
		])->where(['estado_id' => $city->estado_id]);

		return $cities;
    }

    public function generateBasicLocation($columnSize = 'col-md-12', $citiesId = null, $cityId = 'cities', $disabled = false, $extra_classes = '')
    {
        $this->injectScripts();
